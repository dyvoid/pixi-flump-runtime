...
var flump_filters_AnimateTintFilter = function(color,multiplier) {
	if(multiplier == null) {
		multiplier = 1;
	}
	PIXI.Filter.call(this,null,this.getFragmentSrc());
	this.uniforms.color = this.hex2array(color);
...
flump_filters_AnimateTintFilter.prototype = $extend(PIXI.Filter.prototype,{
	...
	,hex2array: function(color) {
		return [_$UInt_UInt_$Impl_$.toFloat(color >>> 16 & 255) / _$UInt_UInt_$Impl_$.toFloat(255),_$UInt_UInt_$Impl_$.toFloat(color >>> 8 & 255) / _$UInt_UInt_$Impl_$.toFloat(255),_$UInt_UInt_$Impl_$.toFloat(color & 255) / _$UInt_UInt_$Impl_$.toFloat(255)];
	}
	,update: function(color,multiplier) {
		if(multiplier == null) {
			multiplier = 1;
		}
		this.color = color;
		this.multiplier = multiplier;
		PIXI.utils.hex2rgb(color,this.uniforms.color);
		this.uniforms.multiplier = multiplier;
	}
...
var pixi_flump_Movie = function(symbolId,resourceId,autoUpdate) {
	if(autoUpdate == null) {
		autoUpdate = true;
	}
...
pixi_flump_Movie.prototype = $extend(PIXI.Container.prototype,{
	...
	,update: function(dt) {
		this.player.advanceTime(dt * this.animationSpeed);
	}
	,tick: function() {
		this.update(this.ticker.elapsedMS * this.animationSpeed);
	}
	,onAdded: function(to) {
...
pixi_flump_Parser.parse = function(resolution,loadFromCache) {
	...
	return function(resource,next) {
		if(resource.data == null || resource.type != PIXI.loaders.Resource.TYPE.JSON) {
			return;
		}
